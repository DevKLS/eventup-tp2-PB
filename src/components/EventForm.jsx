 ...
 * Com validação de domínio para administradores (@eventup.com).
 */
function EventForm({ onEventoAdicionado }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!ehAdministrador) {
      setError(true);
      return setMessage("Acesso negado: Apenas administradores (@eventup.com) podem criar eventos.");
    }
    ...
    try {
      const { error: insertError } = await supabase.from('eventos').insert({ 
        ...
        description: formData.description, // Campo incluído
  ...
  return (
    <form className="contact-form" onSubmit={handleSubmit}>
      ...
      <button 
        ...
        className={`btn-submit-form ${isSubmitting ? "btn-disabled" : ""}`}
      >
